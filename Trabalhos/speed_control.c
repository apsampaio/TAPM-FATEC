 ...
 *
 * Trabalho :
 *  Controlar a velocidade de uma ventoinha em funçao
 *  da temperatura de um sensor
 *  temperatura = 25c | velocidade = 5% 
 */
...
void __interrupt(high_priority) my_isr() {
    ...
    if(INTCONbits.TMR0IF) {           // Realiza as instruções caso a Flag do Timer0 esteja HIGH      
        ...
        float tensao = (valor_adc * 5) / 1023; // 5 = VCC , 1023 valor maximo do ADC
        float temperatura = tensao / 0.010; // dividimos a tensão por 0.010 que representa os 10 mV do sensor de temperatura

        if(temperatura <= 24){
            PR2 = 250; // Desativa duty cycle
        } else if (temperatura >= 61) {
             PR2 = 24; // Duty-Cycle em 100%
        }   
        else {
            // 25 = 5%
            // 60 = 100%;
            PR2 = map(valor_adc, 25, 60, 249, 24); // Converte o valor de 25 a 60 para 1.000HZ a 10.000HZ
        }
...
void DutyCycle(void) {
    // Calculo de Duty Cycle utilizando Equação 15-2
    // Datasheet 4520, pagina 144
...
unsigned int map(unsigned int value, unsigned int fromLow, unsigned int fromHigh, unsigned int toLow, unsigned int toHigh) {   
    // Mesmo algoritmo implementado pelo Arduino para mapeamento de valores
